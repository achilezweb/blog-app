...
<x-app-layout>
    ...
    <div class="py-12">
        ...
        @endif
            ...
            <div class="py-4">
                <form action="{{ route('chatgpts.search') }}" method="GET">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('chatgpts.store') }}">
                        ...
                        <div>
                            <textarea name="prompt" id="prompt" class="block mt-1 w-full" rows="4" placeholder="Enter Question" required autofocus>{{ old('prompt') }}</textarea>
                            {{-- <input type="text" name="prompt" id="prompt" class="block mt-1 w-full" rows="4" placeholder="Enter Question" value="{{ old('prompt') }}" required autofocus> --}}
                            @error('prompt')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>

                        <select name="channel_id" id="privachannel_idcy_id" class="form-control">
                        ...
                        @foreach ($chatgpts as $chatgpt)
                            <tr>
                                ...
                                {{-- <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('chatgpts.edit', $chatgpt) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <form action="{{ route('chatgpts.destroy', $chatgpt) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
