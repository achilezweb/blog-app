        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    <h1 class="text-xl font-semibold">Edit Tags for {{ $post->title }}</h1>
                    <form method="POST" action="{{ route('tag-post.update', $post->id) }}">
                        @csrf
                        @method('PUT')
                        <div>
                            <select name="tag_id[]" id="tag_id" class="form-control block w-full mt-1" multiple>
                                @foreach($tags as $tag)
